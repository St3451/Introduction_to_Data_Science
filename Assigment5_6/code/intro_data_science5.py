...
def stdize_train_test(traindata, testdata):    
    """
...
def whitening(data):
    ...
    Ddata = mds(Cdata, dimensions = n_features)
    Wdata = normalize(Ddata)
    return Wdata
...
def k_nearest_neigh(xtrain, xtest, ytrain, ytest, k=5):
    ...
    for j in range(len(xtest)): 
        # Return a list for the Euclidean distances between a specific point (row) 
        # in the test data set and all points in the train data set                                                     
...
def get_kbest_cv(xtrain, ytrain, kmax=12, kfold=5, return_topk=False):
    """
    ...
    for k in range(1,kmax):
        error, accuracy = cross_validation(xtrain, ytrain, k, kfold)
...
def mds(data, dimensions = 2):
    ...
    data_projected = Q.T @ data.T # Project the data in the new space defined by the eigenvectors 
    return data_projected.T        

# Slighly different function to project the centroids on the eigenvectors obtained from data
def project_centroids(data, centroids, dimensions = 2):
    """
    Obtain eigenvectors (Q matrix) from training data, 
    use it to project the centroids in the new space 
    ...
    # Dimensionality reduction
    Q = eigvect[:,:dimensions] 
    centroids_projected = Q.T @ centroids.T 
    return centroids_projected.T               
